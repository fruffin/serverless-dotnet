    ...
    return new BbPromise((resolve, reject) => {
      try {
        if (self.pluginconfig.assemblyname) {
          self.serverless.cli.log(`[DotNet] Using pre-defined assembly name: '${JSON.stringify(self.pluginconfig.assemblyname)}'`);
        } else {
    ...
    return new BbPromise((resolve, reject) => {
      try {
        if (self.pluginconfig.namespace) {
          self.serverless.cli.log(`[DotNet] Using pre-defined assembly name: '${JSON.stringify(self.pluginconfig.namespace)}'`);
        } else {
    ...
    return new BbPromise((resolve, reject) => {
      try {
        if (self.pluginconfig.entrypointclass) {
          self.serverless.cli.log(`[DotNet] Using pre-defined entrypoint class: '${JSON.stringify(self.pluginconfig.entrypointclass)}'`);
        } else {
          if (!self.entryPointClass) {
            self.setEntrypointClassContent(self);
          }
