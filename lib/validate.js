...
module.exports = {
  ...
  getProjectPathIfExist(self) {
    ...
    if (self.pluginconfig.projectpath) {
      ...
      self.serverless.cli.log(`[DotNet] Using pre-defined project path: '${JSON.stringify(self.pluginconfig.projectpath)}'`);
    }
  },
  getProjectFileIfExist(self) {
    ...
    if (self.pluginconfig.projectfile) {
      ...
      if (!self.projectFile.includes('.csproj')) {
        self.projectFile = `${projectFile}.csproj`;
      }
      self.serverless.cli.log(`[DotNet] Using pre-defined project file: '${JSON.stringify(self.pluginconfig.projectFile)}'`);
    }
  }
};
